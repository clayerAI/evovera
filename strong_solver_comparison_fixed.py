#!/usr/bin/env python3
"""
Strong Solver Comparison: v19 vs OR-Tools - Fixed Version
...
def run_v19_on_instance(parser: TSPLIBParser, seed: int = 42) -> Dict[str, Any]:
    ...
    try:
        # Note: solve() returns (tour, tour_length, runtime_internal)
        tour, tour_length, _ = solver.solve()
        end_time = time.time()
        ...
        for i in range(len(tour)):
            from_node = tour[i]
            to_node = tour[(i + 1) % len(tour)]
            calculated_length += distance_matrix[from_node][to_node]
        
        # Use the calculated length for consistency
...
def main():
    ...
    # Save results
    output_file = "strong_solver_comparison_results_fixed.json"
    ...
    methodology_file = "strong_solver_comparison_methodology.md"
    with open(methodology_file, 'w') as f:
        f.write("# Strong Solver Comparison Methodology\n\n")
        f.write("## Overview\n")
        f.write("This document describes the methodology for comparing v19 Christofides Hybrid Structural ")
        f.write("algorithm against OR-Tools (state-of-the-art TSP solver) on TSPLIB instances.\n\n")
        
        f.write("## Instances\n")
        f.write("- eil51: 51 nodes, EUC_2D distance, optimal=426\n")
        f.write("- kroA100: 100 nodes, EUC_2D distance, optimal=21282\n\n")
        
        f.write("## v19 Algorithm Parameters\n")
        f.write("- percentile_threshold: 70\n")
        f.write("- within_community_weight: 0.8\n")
        f.write("- between_community_weight: 0.3\n")
        f.write("- apply_2opt: True\n")
        f.write("- time_limit: 60.0 seconds\n")
        f.write("- seed: 42 (consistent with previous evaluations)\n\n")
        
        f.write("## OR-Tools Parameters\n")
        f.write("- first_solution_strategy: PATH_CHEAPEST_ARC\n")
        f.write("- local_search_metaheuristic: GUIDED_LOCAL_SEARCH\n")
        f.write("- time_limit: 30 seconds\n\n")
        
        f.write("## Distance Calculation\n")
        f.write("For EUC_2D instances, distances are calculated as rounded Euclidean distances:\n")
        f.write("`dist = round(sqrt(dx² + dy²))`\n\n")
        
        f.write("## Gap Calculation\n")
        f.write("Gap percentage is calculated as:\n")
        f.write("`gap = ((tour_length - optimal) / optimal) * 100%`\n\n")
        
        f.write("## Performance Ratio\n")
        f.write("Performance ratio is calculated as:\n")
        f.write("`ratio = v19_tour_length / ortools_tour_length`\n")
        f.write("A ratio of 1.0 means equal performance, <1.0 means v19 is better (impossible if OR-Tools found optimal),\n")
        f.write(">1.0 means OR-Tools is better.\n")
    
    print(f"Methodology documentation saved to: {methodology_file}")
