...
def run_instance(instance_name, instance_info, results):
    ...
    # Confidence interval (95%) - simplified without scipy
    if len(gaps) > 1:
        # Use z-score approximation for 95% CI (1.96 * std/sqrt(n))
        ci_margin = 1.96 * gap_std / np.sqrt(len(gaps))
        ci_lower = avg_gap - ci_margin
        ci_upper = avg_gap + ci_margin
    else:
