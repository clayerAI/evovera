...
Final version handling dictionary return values.
"""
...
def run_single_instance(algorithm_class, n_customers: int, capacity: float, seed: int) -> float:
    ...
    try:
        ...
        # Solve using solve_cvrp method (returns dict)
        result = vrp.solve_cvrp()
        
        # Extract total_distance from result dict
        if isinstance(result, dict) and 'total_distance' in result:
            return float(result['total_distance'])
        else:
            print(f"Unexpected result format from {algorithm_class.__name__}: {type(result)}")
            return float('inf')
...
def statistical_validation(n_customers_list: List[int], seeds_per_size: int = 10, capacity: float = 50.0):
    ...
    for n_customers in n_customers_list:
        ...
        for seed in range(seeds_per_size):
            ...
            if baseline_dist > 0 and baseline_dist != float('inf') and v21_dist != float('inf'):
