...
Per Vera's critical path reinforcement: Run with 10+ seeds, confidence intervals, p-values.
"""
...
def statistical_validation(n_customers_list: List[int], seeds_per_size: int = 10, capacity: float = 50.0):
    ...
    for n_customers in n_customers_list:
        ...
        v21_mean = np.mean(v21_distances)
        baseline_std = np.std(baseline_distances, ddof=1)
        v21_std = np.std(v21_distances, ddof=1)
        ...
        improvements = []
        for b, v in zip(baseline_distances, v21_distances):
            if b > 0:
        ...
        if improvements:
            ...
            # Paired t-test
            t_stat, p_value = stats.ttest_rel(baseline_distances, v21_distances)
