...
def evaluate_instance(instance_name: str, filepath: str, optimal: float, seeds: int = N_SEEDS) -> Dict:
    ...
    timeout_config = {
        "att532": 300,  # Increased timeout for large ATT instance
        "a280": 180,
        "d198": 120,
        "lin318": 180,
        "pr439": 240
    }
    timeout = timeout_config.get(instance_name, 120)
    ...
    for seed in range(seeds):
        ...
        try:
            ...
            # Run v11 algorithm with timeout
            def solve_func():
                start_solve = time.time()
                tour, length = solver.solve()
                runtime = time.time() - start_solve
                return tour, length, runtime
                return solver.solve()
            
            result = run_with_timeout(solve_func, timeout)
            if result is None:
                print(f"TIMEOUT: Exceeded {timeout}s timeout")
                continue
            
            tour, length, runtime = result
