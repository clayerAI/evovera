...
def test_euclidean_points():
    ...
    # Basic validation
    assert len(tour) == len(points) + 1, f"Tour length mismatch: {len(tour)} != {len(points)+1}"
    assert len(set(tour[:-1])) == len(points), "Tour contains duplicates (excluding endpoint)"
...
def test_hybrid_features():
    ...
    tour, length, runtime = solver.solve()
    print(f"  ✓ Hybrid algorithm executed successfully (tour length: {length:.4f}, runtime: {runtime:.4f}s)")
