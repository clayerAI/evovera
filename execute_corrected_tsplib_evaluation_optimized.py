#!/usr/bin/env python3
"""
Optimized corrected TSPLIB evaluation with vectorized distance calculation.
"""
...
def calculate_distance_matrix_vectorized(points, edge_weight_type):
    """Calculate distance matrix using vectorized operations."""
    if edge_weight_type == "ATT":
        # ATT distance: ceil(sqrt((dx²+dy²)/10))
        # Use cdist with custom metric
        def att_distance(u, v):
            dx = u[0] - v[0]
            dy = u[1] - v[1]
            return np.ceil(np.sqrt((dx*dx + dy*dy) / 10.0))
        
        dist_matrix = cdist(points, points, metric=att_distance)
    else:
...
def main():
    print("=" * 80)
    print("OPTIMIZED CORRECTED TSPLIB EVALUATION")
    ...
    for instance_name, filepath, optimal in instances:
        ...
        dist_matrix = calculate_distance_matrix_vectorized(points, parser.edge_weight_type)
    ...
    output_file = "corrected_tsplib_evaluation_results_optimized.json"
