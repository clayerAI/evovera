    ...
    if matches:
        ...
        if instance in results['instances']:
            ...
            if v11_gaps and gaps:
                ...
                # Perform t-test (only if we have multiple seeds)
                if len(v11_gaps) == len(gaps) and len(v11_gaps) > 1:
                    t_stat, p_value = stats.ttest_rel(v11_gaps, gaps)
                    results['instances'][instance]['comparison']['t_statistic'] = float(t_stat)
                    results['instances'][instance]['comparison']['p_value'] = float(p_value)
                    results['instances'][instance]['comparison']['significant'] = bool(p_value < 0.05)
    else:
