...
class ChristofidesHybridStructuralOptimized:
    ...
    def _detect_communities(self, mst_adj: List[List[Tuple[int, float]]], 
        ...
        """
        Detect communities in MST using edge centrality.
        
        Removes high-centrality edges to partition tree into communities.
        """
        n = self.n
        ...
        threshold = np.percentile(centrality_values, percentile_threshold)
...
def solve_tsp(points: List[Tuple[float, float]], 
              distance_matrix: Optional[Union[List[List[float]], np.ndarray]] =              distance_matrix: Optional[Union[List[List[float]], np.ndarray]] = None,
