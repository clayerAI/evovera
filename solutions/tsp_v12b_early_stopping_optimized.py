...
class ChristofidesHybridStructuralOptimizedV11:
    ...
    def _two_opt(self, tour: List[int]) -> List[int]:
        """Apply 2-opt local optimization with improvement threshold early stopping.
        
        Optimization: Stop when per-iteration improvement drops below 0.01% threshold
        to skip final iterations with diminishing returns while preserving quality.
        """
        n = len(tour) - 1  # Exclude closing node
        improved = True
        iteration = 0
        current_length = self._compute_tour_length(tour)
        IMPROVEMENT_THRESHOLD = 0.00001  # 0.001% improvement threshold
            ...
            for i in range(1, n - 1):
                for j in range(i + 1, n):
                    ...
                    if new < current - 1e-9:
                        ...
                        improved = True
                        current_length -= (current - new)
                        break
                if improved:
                    break
            
            # Early stopping: if improvement is below threshold, stop
            if improved:
                improvement_ratio = (prev_length - current_length) / prev_length
