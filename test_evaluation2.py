...
if parser2.parse():
    ...
    # Test distance matrix calculation
    dist_matrix2 = parser2.get_distance_matrix()
    print(f"  Distance matrix shape: {dist_matrix2.shape}")
    print(f"  Sample distance [0,1]: {dist_matrix2[0,1]:.2f}")
    ...
    # Verify the distance matrix matches ATT calculation
    manual_euclidean = np.sqrt(dx*dx + dy*dy)
    print(f"  Manual Euclidean [0,1]: {manual_euclidean:.2f}")
    print(f"  Matrix/Euclidean ratio: {dist_matrix2[0,1]/manual_euclidean:.2f}")
else:
